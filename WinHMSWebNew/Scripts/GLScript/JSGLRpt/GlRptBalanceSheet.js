...
var PageLoad = function () {
    ...
    webix.ui({
        view: "button", id: "btnExcel", value: "Excel", width: 30, css: 'webix-secondary', container: "divExcel", label: excel, tooltip: true,
        on: {
            onItemClick: function () {
                fnExcelExport();
...
function fnLoadGrid() {
    ...
    try {
        ...
        var vFromDt = formatDate1(window.FiscalStartDT);
        ...
        var MultiComp = "";
        var ReqType = "GET_FNLOADBALANCESHEETVER";
        if (bMulti == "1") {            

            //MultiComp = $$("Property").getValue();
            MultiComp = fnRetComboVal($$("Property"), "ORGID");
            ReqType = "GET_FNLOADBALANCESHEETVERMCOMP";
        }
...
function fnRowDblClick(Item) {
    debugger;
    var RowId = Item.row;
    var ColId = Item.column;
    ...
    if (ColId.includes("PRV") == true) return;

    //var ToDt = ColId.substr(ColId.length - 10);
    //var FromDt = "";

    //var PeriodData = $$("dtPeriodPop").serialize(true);

    //var newData = PeriodData.filter(function (el) {
    //    return el.PERIOD_END_DT == ToDt;
    //});

    //if (newData.length > 0) {
    //    FromDt = newData[0].PERIOD_START_DT
    //}
    ...
    //if ($$("chkAllPeriod").getValue() == "1") FromDt = window.FiscalFromDt;
    //else FromDt = $$("FromDt").getValue();
   var FromDt = window.FiscalFromDt;

    var ToDt = "";
    if ($$("chkAllPeriod").getValue() == "1") ToDt = window.FiscalToDt;
    else ToDt = $$("ToDt").getValue();
    ...
    if (AC_ID != "" && AC_ID != "00050002" && AC_ID != "0005" && AC_ID != "00050003" && AC_ID != "0006" && AC_ID != "00050001" && AC_NM!="Total") {
        $.ajax({
            type: "POST",
            url: "/GLReports/GroupTrialBalancePopup",
            cache: false,
            charset: 'utf-8',
            data: "COMP_ID=" + CompId + "&Division=" + Division + "&From=" + FromDt + "&To=" + ToDt + "&AC_NM=" + AC_NM + "&AC_ID=" + AC_ID,
            success: function (data) {
                GlBSWindowLoad(AC_CAT);
            }
        });
...
function GlBSWindowLoad(AC_CAT) {
    //debugger;
