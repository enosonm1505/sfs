...
var PageLoad = function () {
    ...
    webix.ui({
        container: "divBtnAddCol", id: "btnAddCol", css: "webix_primary", view: "button", inputWidth: 100, label: "Add Column", click: function () {
            debugger;
            if ($("#HdnPageLoad").val() == "1") {
                for (j = 0; j < 10; j++) {
                    var Col = "";
                    if (j == 0) Col = "ix0";
                    if (j == 1) Col = "ix31";
                    ...
                    if (j == 8) Col = "ix14";
                    if (j == 9) Col = "ix15";
                    if ($$("gridRpt").getColumnConfig(Col) != undefined) {
                        var data = $$("gridRpt").getColumnConfig(Col).header[0].text;
                        var str = [];
                        str = data.split('-');
                        var dtAdd = $$("dtAddColPop").serialize();
                        var lenval = dtAdd.length;
                        if (lenval != 0) {
                            if ($.trim(str[1]) != "") {
                                dtAdd[j].ixFrom = $.trim(str[0]);
                                dtAdd[j].ixTo = $.trim(str[1]);
                            }
    ...
    webix.ui({
        ...
        container: "divBtnDelCol", id: "btnDelCol", css: "webix_primary", view: "button", inputWidth: 100, label: "Del Column", click: function () {
            debugger;
            GridClear();


            var colunm = $$("gridRpt").config.columns;

            var lencolumns = colunm.length;
            if (lencolumns > 6) {
                colunm.pop();
                $$("gridRpt").refreshColumns();
            }
            if (lencolumns > 7 && $("#HdnDelColValue").val() == "2") {

                var SelRow = $$("dtAddColPop").getSelectedId(false);
                $$("dtAddColPop").editStop();
                if (SelRow == undefined || SelRow == null) SelRow = $$("dtAddColPop").getLastId();
                $$("dtAddColPop").refresh();
                var dtAdd = $$("dtAddColPop").serialize();
                for (j = 0; j < 10; j++) {
                    var lenval = dtAdd.length;
                    if (lenval != 0 && (dtAdd[j].ixFrom == "" || dtAdd[j].ixFrom == undefined)) {
    ...
    webix.ui({
        ...
        body: {
            rows: [{
                ...
                height: 305,
                columns: [
                    { header: ["From Days"], id: "ixFrom", width: 120, format: webix.i18n.intFormat, css: { 'text-align': 'right ! important' }, },  //editor: 'text', liveEdit: true
                    { header: ["To Days"], id: "ixTo", width: 120, format: webix.i18n.intFormat, css: { 'text-align': 'right ! important' }, editor: 'text', liveEdit: true },
                ],
                data: [{ "ixFrom": 0, "ixTo": 30 }, {}, {}, {}, {}, {}, {}, {}, {}],

                on: {

                    'onItemClick': function (id) {

                        var getval = $$("dtAddColPop").getItem($$("dtAddColPop").getSelectedId() - 1);
                        var RowId = $$("dtAddColPop").getItem($$("dtAddColPop").getSelectedId());
                        var data = getval.ixTo;
                        if (!isNaN(data)) {
                            if (data != "") data = parseInt(data) + 1;
                            var currRowId = parseInt($$("dtAddColPop").getSelectedId());
                            var item = $$("dtAddColPop").getItem(currRowId);
                            item.ixFrom = data;
                            $$("dtAddColPop").updateItem(currRowId, item)
                            $$("dtAddColPop").refresh();
                        }
                    },
                    ...
                    'onKeyPress': function (e, id) {
                        debugger;
                        if (e == 40) {
                            var RowId = parseInt($$("dtAddColPop").getSelectedId()) + 1;
                            var getval = $$("dtAddColPop").getItem($$("dtAddColPop").getSelectedId());
                            var data = getval.ixTo;
                            if (data != "") data = parseInt(data) + 1;
                            var item = $$("dtAddColPop").getItem(RowId);
                            item.ixFrom = data;
                            $$("dtAddColPop").updateItem(RowId, item);
                            $$("dtAddColPop").refresh();                        
                        }
                    },
                    'onBlur': function () {
                    ...
                    'onAfterEditStop': function (state, editor) {
                        debugger;
                        $$("dtAddColPop").refresh();
                        var dtAddColPop = $$("dtAddColPop").getSelectedItem(true);
                        if (editor.column == 'ixFrom') {
                            var ixfrm = parseInt(dtAddColPop[0].ixFrom);
                            if (isNaN(ixfrm)) {
                                dtAddColPop[0].ixFrom = '';
                                $$("dtAddColPop").refresh();
                            }
                        }
                        else if (editor.column == 'ixTo') {

                            var getval = this.getItem(editor.row);
                            if (getval.ixTo != '') {
                                var ixto = parseInt(getval.ixTo);
                                var ixfrm = parseInt(getval.ixFrom);
                                if (isNaN(ixto) || isNaN(ixfrm)) {
                                    getval.ixTo = '';
                                    dtAddColPop[0].ixTo = '';
                                    $$("dtAddColPop").refresh();
                                     if (isNaN(ixfrm))
                                        webix.message({ type: 'warning', text: 'Invalid From Days' });
                                }
                            }
                        }
                ...
                cols: [
                    {
                        ...
                        click: function () {
                            ...
                            set = { id: "ixPropId", header: 'PropertyId', hidden: true };
                            ColVal.push(set);

                            set = { id: "ixPropNm", header: 'Property', hidden: true, width: 150, css: { 'text-align': 'left ! important', }, exportType: "string" };
                            ColVal.push(set);

                            set = { id: "ixBillNo", header: 'Vouch No.', width: 90, css: { 'text-align': 'center ! important', }, exportType: "string" };
                            ColVal.push(set);

                            set = { id: "ixVtype", header: { text: "Voucher Type", }, width: 120, css: { 'text-align': 'left ! important', }, exportType: "string", hidden: true };
                            ColVal.push(set);

                            set = { id: "ixBillDt", header: 'Vouch Dt', width: 90, css: { 'text-align': 'center ! important', }, };
                            ColVal.push(set);

                            set = { id: "ixRefTy", header: { text: "Ref Type", }, width: 120, css: { 'text-align': 'left ! important', }, exportType: "string" };
                            ColVal.push(set);

                            set = { id: "ixRefNm", header: { text: "Ref Name", }, width: 140, css: { 'text-align': 'left ! important', }, exportType: "string" };
                            ColVal.push(set);

                            set = { id: "ixDueDt", header: 'Due Dt', width: 120, css: { 'text-align': 'center ! important', }, };
                            ColVal.push(set);

                            set = { id: "ixBal", header: 'Balance Amt', width: 120, css: { 'text-align': 'right ! important', }, };
                            ColVal.push(set);

                            set = { id: "ixHid", header: { text: "Trn_Id", }, hidden: true };
                            ColVal.push(set);

                            set = { id: "ixHid1", header: { text: "Trn_Id_Srno", }, hidden: true };
                            ColVal.push(set);

                            set = { id: "ixRef_SNo", header: { text: "Ref Sno", }, hidden: true };
                            ColVal.push(set);

                            set = { id: "ixRef_Ty_ID", header: { text: "Ref Ty Id", }, hidden: true };
                            ColVal.push(set);

                            set = { id: "ixTrnTyId", header: { text: "Trn Ty Id", }, hidden: true };
                            ColVal.push(set);
                            set = { id: "ixAc_ID", header: { text: "Acc Id", }, hidden: true };
                            ColVal.push(set);

                            set = { id: "CLR", header: { text: "CLR", }, hidden: true };
                            ColVal.push(set);

                            var data = $$("dtAddColPop").serialize();
                            var Lenaddcol = data.length;
                            var dyVal = true;
                            for (var i = 0; i < Lenaddcol; i++) {

                                if ($.trim(data[i].ixFrom) != undefined && $.trim(data[i].ixFrom) != "" && $.trim(data[i].ixTo) != undefined && $.trim(data[i].ixTo) != "") {

                                    var fromdays = $.trim(data[i].ixFrom);
                                    if ($.trim(data[i].ixTo) != "") {
                                        var todays = $.trim(data[i].ixTo);
                                    }
                                    var Hdr = fromdays + "-" + todays;
                                    if (i == 0) {  //ix0
                                        set = { id: "ix0", width: 120, css: { 'text-align': 'center ! important', }, exportType: "string", header: { text: Hdr }, };
                                        ColVal.push(set);
                                    }
                                    if (i == 1) {
                                        set = { id: "ix31", width: 120, css: { 'text-align': 'center ! important', }, exportType: "string", header: { text: Hdr }, };
                                        ColVal.push(set);
                                    }
                                    if (i == 2) {
                                        set = { id: "ix61", width: 120, css: { 'text-align': 'center ! important', }, exportType: "string", header: { text: Hdr }, };
                                        ColVal.push(set);
                                    }
                                    if (i == 3) {
                                        set = { id: "ix91", width: 120, css: { 'text-align': 'center ! important', }, exportType: "string", header: { text: Hdr }, };
                                        ColVal.push(set);
                                    }
                                    if (i == 4) {
                                        set = { id: "ix11", width: 120, css: { 'text-align': 'center ! important', }, exportType: "string", header: { text: Hdr }, };
                                        ColVal.push(set);
                                    }
                                    if (i == 5) {
                                        set = { id: "ix12", width: 120, css: { 'text-align': 'center ! important', }, exportType: "string", header: { text: Hdr }, };
                                        ColVal.push(set);
                                    }
                                    if (i == 6) {
                                        set = { id: "ix13", width: 120, css: { 'text-align': 'center ! important', }, exportType: "string", header: { text: Hdr }, };
                                        ColVal.push(set);
                                    }
                                    if (i == 7) {
                                        set = { id: "ix14", width: 120, css: { 'text-align': 'center ! important', }, exportType: "string", header: { text: Hdr }, };
                                        ColVal.push(set);
                                    }
                                    if (i == 8) {
                                        set = { id: "ix15", width: 120, css: { 'text-align': 'center ! important', }, exportType: "string", header: { text: Hdr }, };
                                        ColVal.push(set);
                                    }
                                    if (i == 9) {
                                        set = { id: "ix16", width: 120, css: { 'text-align': 'center ! important', }, exportType: "string", header: { text: Hdr }, };
                                        ColVal.push(set);
                                    }
                                }
                                else if (dyVal == true && i != 0) {
                                    var todays = $.trim(data[i - 1].ixTo);
                                    set = { id: "ix181", width: 120, css: { 'text-align': 'center ! important', }, exportType: "string", header: { text: ">" + todays }, };
                                    ColVal.push(set);
                                    dyVal = false;
                                    if ($.trim(data[i].ixFrom) != "") {
                                        data[i].ixFrom = "";
                                        $$("dtAddColPop").refresh();
                                        break;
                                    }
...
function GridDesign() {
    webix.ui({
        ...
        columns: [
                ...
                { id: "ixBillNo", header: 'Vouch No.', width: 90, css: { 'text-align': 'center ! important', }, exportType: "string" },//120
                { id: "ixVtype", header: { text: "Voucher Type", }, width: 120, css: { 'text-align': 'left ! important', }, exportType: "string", hidden: true },
                { id: "ixBillDt", header: 'Vouch Dt', width: 90, css: { 'text-align': 'center ! important', }, },//120
                ...
                {
                    id: "ix181", header: ' > 30', width: 120, css: { 'text-align': 'right ! important', },
                },
...
function fnRowDblClick(RowId) {
    ...
    if (TRN_ID != "" && TRN_ID != null) {
        if (TRN_ID_SRNO != "" && TRN_ID_SRNO != null) {
            ...
            // Window1 = window.open("/GLTransaction/GlReversalEntry/?RPage=" + "1" + "&PARTIAL=" + "1" + "&NewCmpId=" + CompId + "&ID=" + UrlQryStr + "", "DayBookOpenBillRef", "width=" + SW + ",height=" + Sh + ",left=50,top=20 ");
            var PageUrl = "/GLTransaction/GlReversalEntry/?RPage=" + "1" + "&PARTIAL=" + "1" + "&NewCmpId=" + CompId + "&ID=" + UrlQryStr + "";
        ...
        webix.ui({
            ...
            on: {
                onShow: function () {
                    ...
                    $$('DrillDayBookPopup').define("width", vWidth);
                    $$('DrillDayBookPopup').define("height", vHeight)
                    $$('DrillDayBookPopup').resize();
                }
            },
            body: {
                rows: [{
...
function btnFilterClick() {
    ...
    $$("chkAllBill").setValue($$("txtAllBill").getValue());
...
function btnOkFilterClick() {
    ...
    $$("txtAllBill").setValue($$("chkAllBill").getValue());
...
function fnLoadGrid() {
    ...
    var ReqNm = "";
    ReqNm = "GET_FNGLLOADCUSTOUTSTANDBILLS"

    var dataDyDays = $$("dtAddColPop").serialize();
