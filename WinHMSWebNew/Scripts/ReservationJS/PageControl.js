...
var ControlCall = function (request, prop_id) {
    debugger;
    var ddlids = [];
    var modddlids = [];
    var PromptIds = [];
    ...
    $.ajax({
        ...
        success: function (data) {         
            ...
            $.each(Dropdownlist, function (key, value) {
                ...
                if (elelm === undefined) {
                    if (key == "ddltitle") {                    
                        $("#" + key).kendoDropDownList({ dataTextField: "ITEM_NM", dataValueField: "ITEM_VAL", dataSource: value, height: 100, optionLabel: " ", open: adjustDropDownWidth });
                        var title =$("#ddltitle").data('kendoDropDownList');
                        title.wrapper.hide();
                    }
              else {
                        $("#" + key).kendoDropDownList({ dataTextField: "ITEM_NM", dataValueField: "ITEM_VAL", dataSource: value, optionLabel: " "});
                  }
                }
                else {
                    $("#" + key).kendoDropDownList({ dataTextField: "ITEM_NM", dataValueField: "ITEM_VAL", dataSource: value, optionLabel: " ", });
                ...
                var ddlcurrency = window.Dropdownlist.ddlcurrency;
                var ddlTariffCurrency = $("#ddlTariffCurrency").kendoDropDownList({
                autoBind: false,
                dataTextField: "ITEM_NM",
                dataValueField: "ITEM_VAL",
                dataSource: ddlcurrency,
                open: adjustDropDownWidth,
                attributes: { style: "height:10px;" },
                template: "<span data-id='${data.ITEM_VAL}'>${data.ITEM_NM}</span>"
                });          
                $("#ddlTariffCurrency").data("kendoDropDownList").value(pgindic.BASE_CURRENCY_ID);
