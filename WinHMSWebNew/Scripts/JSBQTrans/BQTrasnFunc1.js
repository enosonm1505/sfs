...
function fnFBRowAdd(option) {
    ...
    var addrow = {
        ...
        chkTaxInd: '', vTaxAmt: '', ChkVentaxInd: '0', vVentaxAmt: '', hdnvDSV: '', hdnVenueId: '', hdnfbkids: '', hdndriver: '',
        hdnPkgNm: '', FRowId: '',
    };
    ...
    if ($.trim(option) == "0") {
        ...
        if (dtFB.length != 0) {
            ...
            for (i = 0; i < dtFB.length; i++) {
                ...
                var vActPax1 = ($.trim(grdData[i].vActPax) == "" ? 0 : parseInt(grdData[i].vActPax));

                var vExpPax1 = ($.trim(grdData[i].vExpPax) == "" ? 0 : parseInt(grdData[i].vExpPax));
            ...
            elements: [
                {
                    ...
                    scroll: "y",
                    columns: [
                            { header: ["Reserve#", { content: "textFilter" }, ], id: "RsvNo", width: 70, css: { 'text-align': 'Center ! important' } },
    ...
    $.ajax({
        ...
        success: function (d) {
            if (d != "") {
                ...
                if (SelResData[0].CG_NM != "" && SelResData[0].CG_TM != "") {
...
function fnFBValidate()
{
    ...
    if (grdData.length != 0) {
        ...
        for (i = 0; i < grdData.length; i++) {
            ...
            var vActPax1 = (grdData[i].vActPax == "" ? 0 : parseInt(grdData[i].vActPax));
            ...
            if (vActPax1 == 0 && grdData[i].hdnPlnPkgId != "") {
                AlertMessage("Guaranteed Pax Cannot be empty");
                return false;
            }
            else if (vActPax1 == 0 && grdData[i].hdnPlnPkgId != "") {
                AlertMessage("Please enter Guaranteed Pax");
                return false;
            }
            ...
            if (parseInt(vExpPax1) < parseInt(vActPax1) && grdData[i].hdnPlnPkgId != "") {
                AlertMessage("Expected Pax should be Greater or equal to Guaranteed Pax");
                return false;
            }
    ...
    if (dtOthers.length > 0) {
        for (j = 0; j < dtOthers.length; j++) {
            ...
            if (OTyId != "" && ItemNM != "") {
                if (Qty == 0) {
                    $$("BQTabView").getTabbar().setValue("MenuFrm");
                    AlertMessage("Qty Cannot by Zero");
                    return false;
                }
            }
        }
    ...
    if (grdData.length != 0 && grdData.length != 1) {
        for (i = 0; i < grdData.length; i++) {
            for (j = 0; j < grdData.length; j++) {
                ...
                if (i != j) {
                    if ($.trim(grdData[j].vSessionId) == $.trim(grdData[i].vSessionId)) {
