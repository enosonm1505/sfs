...
(function (global, factory) {
	...
	webix.protoUI({
	  ...
	  defaults: {
	    ...
	    filter: function (item, value) {
	      var editor = webix.$$(this.config.editor);
	  ...
	  $init: function () {
	    ...
	    this.attachEvent("onBeforeShow", function (node) {
	      if (node.tagName) {
	        var editor = webix.$$(_this.config.editor);
	        var text = editor.getValue(); //only for formulas

	        if (!text || text.charAt(0) !== "=") return false;
	        var sizes = webix.html.offset(node);
	        var textWidth = webix.html.getTextSize(text, "webix_ssheet_formula_input").width;
	        var selectionStart = editor.getCursor();
	        var y = sizes.y + sizes.height;
	        var x = sizes.x + selectionStart * textWidth / text.length;
	...
	webix.protoUI({
	  ...
	  $init: function (config) {
	    ...
	    this.attachEvent("onAfterRender", function () {
	      ...
	      this.inputEv = webix.event(node, ev, function () {
	        _this2._cursor = _this2.getCursor(); //we can start typing on colored span, so we should update input highlight

	        _this2.highlightCells(_this2.getValue());

	        _this2.setCursor(_this2._cursor);
	      });
	      this.keydownEv = webix.event(node, "keydown", function (e) {
	        _this2.endEdit(e);
	      });
	      webix.ui({
	  ...
	  highlightCells: function (value) {
	    var html = "";
	    var master = this.getTopParentView();

	    this._highlightedCells.forEach(function (cell) {
	    ...
	    if (value && value[0] == "=") {
	      ...
	      for (var i = 0; i < parts.length; i++) {
	        if ((i + 1) % 2 != 0) html += parts[i];else {
	          ...
	          var cellText = crossheet ? parts[i].join("!") : parts[i];
	          ...
	          if (activeSheet || crossheet && parts[i][0] == sheet) {
	            ...
	            if (parts[i + 1] == ":" && parts[i + 2] || rangeName) {
	              ...
	              if (rangeName) {
	                var cells = ranges[parts[i]].split(":");
	              ...
	              html += "<span class=\"webix_ssheet_highlight_color_".concat(colorIndex, "\">").concat(rangeName ? parts[i] : cellText + ":" + nextCellText, "</span>");

	              for (var row = Math.min(cell[0], nextCell[0]); row <= Math.max(cell[0], nextCell[0]); row++) {
	                for (var col = Math.min(cell[1], nextCell[1]); col <= Math.max(cell[1], nextCell[1]); col++) {
	                  this.addCellCss(row, col, colorIndex);
	                }
	              }
	            } //cell
	            else {
	                html += "<span class=\"webix_ssheet_highlight_color_".concat(colorIndex, "\">").concat(cellText, "</span>");
	                this.addCellCss(cell[0], cell[1], colorIndex);
	              }

	            var colors = 7;
	            colorIndex += colorIndex == colors ? -1 * (colors - 1) : 1;
	          } else html += webix.isArray(parts[i]) ? parts[i].join("!") : parts[i];
	        }
	      }
	    } else html = value;
	    ...
	    var cursor = this.getDocumentCursor();
	    if (webix.isUndefined(cursor)) return;
	    var cursorNode = cursor.startContainer;
	    if (this.getInputNode().contains(cursorNode)) this.paintValue();
	  },
	  addCellCss: function (row, col, index) {
	  ...
	  focus: function () {
	    ...
	    if (!webix.isUndefined(cursor) && this.getInputNode().contains(cursor.startContainer)) {
	      ...
	      if (length) this.setCursor(length);else this.getInputNode().focus();
	    }
	  },
	  endEdit: function (e) {
	  ...
	  updateCellValue: function () {
	    ...
	    var master = this.getTopParentView();
	    var cell = this.config.activeCell;
	    if (!cell) this.setValue("");else if (newv != master.getCellValue(cell.row, cell.column)) {
	  ...
	  setValueHere: function (value) {
	    ...
	    if (formula && formula.charAt(0) === "=") {
	      ...
	      var updatedStr = str1.replace(/([a-zA-Z(]+)$/, value); //suggest called via up/down key
	  ...
	  setCursor: function (pos) {
	    this._cursor = pos;
	    var symbolsBefore = 0;
	    var childs = this.getInputNode().childNodes;

	    for (var i = 0; i < childs.length; i++) {
	      var textLength = childs[i].length || childs[i].innerText.length;
	  ...
	  getCursor: function () {
	    ...
	    if (cursor) {
	      var pos = cursor.startOffset;
	      var symbolsBefore = 0;
	      var childs = this.getInputNode().childNodes;

	      for (var i = 0; i < childs.length; i++) {
	        var parent = cursor.startContainer.parentNode;
	        if (childs[i] == (parent.tagName == "SPAN" ? parent : cursor.startContainer)) return symbolsBefore + pos;
	...
	function ready$2(view) {
	  ...
	  function startEdit(table, cell, clear) {
	    var cellEditor = table.getColumnConfig(cell.column).editor; //do not interfere with custom editors
	  ...
	  function pasteRange(view, st, en, cell) {
	    ...
	    if (editor._activeMath) {
	      ...
	      if (editor._activeMath != activeSheet) sheet = activeSheet + "!";
	    }

	    var range = st == en ? sheet + st : "".concat(sheet + st, ":").concat(sheet + en);
	    var updateRange = editor._update_range;
	    if (updateRange && updateRange.pos == editor._cursor - updateRange.len) editor.setRange(range, true);else if (editor.expectOperator()) editor.setRange(range);else return endEdit(cell);
	    webix.delay(function () {
	      return editor.focus();
	    });
	    return false;
	...
	function _showRow(action, start, end, view, silent) {
	  ...
	  if (split + 1 >= start.row) view.freezeRows(split);
	}
	...
	function _hideRow(action, start, end, view, silent) {
	  ...
	  if (split >= start.row) view.freezeRows(split);
	...
	function freezeColumns(column) {
	  ...
	  var index = base;
	  if (column && column != this._frozenColumns) index = grid.getColumnIndex(column * 1 + 1);
	...
	function freezeRows(row) {
	  ...
	  var index = 0;
	  if (row && row != this._frozenRows) index = grid.getIndexById(row * 1 + 1);
	  if (index < 0) index = 0;
	  group.set(function () {
