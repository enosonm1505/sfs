...
$(document).ready(function () {  
  ...
  $("#SaveGridRow").click(function (e) {
        ...
        var grid = $("#grid").data('kendoGrid');
        var BankAc_ID = $("#AccountDtval").val();
        var TranTyID = $("#Trantype").val();
        var TotalAmt = $("#DepitTotal").text();
        ...
        for (; it < total; it++) {
            if (grid.dataSource._data[it].AC_NM != "ACCOUNT") {
        ...
        if (ACCountNm != "") {
            if (TranType != "" && TranType != null) {
                if (TransDate != "" && TranType != "" && total != 0) {
                    if (grid._data[no].Depit != 0 || grid._data[no].Depit != "") {
                    ...
                    else if (TransDate != "" && TranType != "" && total != 0 && grid.dataSource._data[no].Depit == 0 || grid.dataSource._data[no].Depit == "") {
                        if (TranType == "2") {
                ...
                $("#AlertMessageHdn").val(" Transaction Type cannot be Empty...!");
                $("#alertType").val('fail');
                AlertMesaage();
            }
            ...
            $("#AlertMessageHdn").val(" Account cannot be Empty...!");
            $("#alertType").val('fail');
            AlertMesaage();
        }
...
function maingridrowadd() {
    ...
    if (TranType != "" && TranType != null) {
        ...
        $("#AlertMessageHdn").val('TRnsaction Type Cannot be Empty...!');
        $("#alertType").val('fail');
        AlertMesaage();
    }
}
                    ...
                    if (DepitVal != 0 || CreditVal != 0) {
                        if (grid.dataSource._data[no].BillDeatil_IND == 1) {
                            ...
                            if (grid1.dataSource.data().length == 0 || grid1.dataSource.data().length == undefined) {
                                ...
                                if (DepitV != "" && CreditV == "") {
                                    $("#differenceAmount").text("D/C Difference: " + Number(DepitV).toFixed(2));
                                }
                                else if (DepitV == "" && CreditV != "") {
                                    $("#differenceAmount").text("D/C Difference: " + Number(CreditV).toFixed(2));
                                }
                            }
                            else {
                    ...
                    if (no1 == no) {
                        ...
                        if (grid.dataSource.data().length != 0 || grid.dataSource.data().length != undefined) {
                            ...
                            if (0 >= TotalValue && DepitVal != 0 || CreditVal != 0) {
                                ...
                                if (grid.dataSource._data[no].AC_NM != null && grid.dataSource._data[no].AC_NM != "") {
                                    dataSource.add({ DC: dropdownval, AC_NM: null, Depit: "", Credit: "", ConvertButton: conButton, Currency: FirstRowCurrency, checkDate: TransDate, RefrDate: TransDate, DocDate: TransDate, RowIndexHiden: total });
                                    if (FirstRowCurrency == DefaultCurrency) {
                                        $("#ForeignCurrencyDIV").hide();
                                    }
                                    else if (FirstRowCurrency != DefaultCurrency) {
