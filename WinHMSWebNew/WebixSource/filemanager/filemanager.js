...
(function (global, factory) {
	...
	function loadDynData(view, url, obj, mode, open) {
	  ...
	  if (view.callEvent("onBeforeDynLoad", [url, obj, mode, open])) {
	    if (url.load) return url.load(null, {
	      action: mode,
	      source: obj.id
	    }).then(function (data) {
	      view.hideProgress();
	...
	function loadSearchData(view, url, id, value) {
	  ...
	  if (view.callEvent("onBeforeSearchRequest", [id, params])) {
	    if (url.load) return url.load(null, params).then(function (data) {
	      view.hideProgress();
	      data = convertData(view, data);
	...
	function convertData(view, data) {
	  if (data && typeof data.text === "function") data = data.text();
	...
	function makeSaveRequest(view, url, obj, callback) {
	  if (view.callEvent("onBeforeRequest", [url, obj])) {
	    ...
	    if (url.load) {
	      url.load(null, webix.copy(obj)).then(function (data) {
	        if (data && typeof data.text === "function") data = data.text();
	        data = view.data.driver.toObject(data);
	        hideSaveMessage(view);
	        if (view.callEvent("onSuccessResponse", [obj, data]) && callback) callback.call(view, obj, data);
	      }, function (error) {
	        if (view.callEvent("onErrorResponse", [obj, error])) errorHandler(view, error);
	      });
	    }
	  }
	}
	...
	function init$c(view, settings) {
	  ...
	  settings.modes.forEach(function (mode) {
	    if (mode == "table") options.push(getModeConfig(mode, "table"));else if (mode == "files") options.push(getModeConfig(mode, "icons"));
	  });
	  var config = {
	    view: "segmented",
	    width: 70,
	  ...
	  if (tree) {
	    ...
	    if (!view.config.readonly) {
	      ...
	      tree.attachEvent("onBeforeMenuShow", function (id) {
	        ...
	        var context = menu.getContext();
	        var type = "";
	        if (context.id) type = this.getParentId(context.id) ? "folder" : "root";
	        menu.filter(function (obj) {
	          var res = true;
	          ...
	          if (obj.batch) {
	            if (!type) {
	              ...
	              res = obj.batch == type;
	              if (type != "root") res = res || obj.batch == "item" || obj.batch == "empty";
	            }
	          }
	...
	function setUploadHandlers(view) {
	  ...
	  if (uploader) {
	    ...
	    uploader.attachEvent("onFileUploadError", function (item, response) {
	      if (view.callEvent("onErrorResponse", [item, response])) errorHandler(view, response);
	      view.hideProgress();
	    });
	  }
	}
	...
	webix.protoUI({
	  ...
	  handlers_setter: function (handlers) {
	    if (typeof handlers == "string") handlers = this.setDefaultHandlers(handlers);
