...
$("#differenceAmount").text("Debit/credit Difference: " + Number(0).toFixed(2));
...
$(document).ready(function () {
    ...
    $("#DepitTotal").attr('disabled', true);
    $("#CreditTotal").attr('disabled', true);
    ...
    $("#FinalDiffAMT").text("Debit/credit Difference: " + Number(finalDiff).toFixed(2));
    $("#DepitTotal").text(Number(depitfinTotal).toFixed(2));
    $("#CreditTotal").text(Number(CreditfinTotal).toFixed(2));

    $("#addrowopen").click(function (e) {
        alert(0);
        NewGridRow(e);
    });

    $("#deleterowopen").click(function (e) {
        CancelGridRow(e);
    });
    ...
    $("#SaveGridRow").click(function (e) {
        ...
        paramValue = JSON.stringify({ TransList: gridData, BillList: gridData2, TranTyID: TranType, TranDate: TransDate, Division: Division, CommaonNarrtion: CommonNarration, VouchNo: VoucherNo });
        var dataSource = grid.dataSource;
        var total = grid.dataSource.data().length;
        if (total == undefined) {
            total = 0;
        ...
        var dd = TotalValue.replace("Debit/credit Difference: ", "");
        TotalValue = dd;
                    ...
                    if (TotalValue == 0) {
                        if (CommonNarration == "" && COMMON_NARR_APPL == 1) {
                            ...
                            if (TestVal == 0) {
                                ...
                                $("#AlertMessageHdn").val("Debit/Credit Value does not Match with the Pending Bill Settlement(s) Made.");
                                $("#alertType").val('fail');
                                 AlertMesaage();
...
function NewGridRow(e) {
    ...
    if (grid.dataSource.data().length == 0 || grid.dataSource.data().length == undefined) {
        ...
        if (0 >= TotalValue && grid.dataSource._data[no].Depit != 0 || grid.dataSource._data[no].Credit != 0) {
            ...
            $("#AlertMessageHdn").val("Debit/Credit Value(s) found Empty.");
            $("#alertType").val('fail');
             AlertMesaage();          
        }
    }
...
function edit(e) {
   ...
   var no = rowIndex;
    ...
    if ((objOffsetVersion = objAgent.indexOf("Chrome")) != -1) {
        ...
        if (grid.dataSource._data[no].DC == "CR" && grid.dataSource._data[no].Currency == "RS") {
            $(e.container).find('input[name="Depit"]').attr("readonly", true);
            $(e.container).find('input[name="Credit"]').focus();
        }
        else if (grid.dataSource._data[no].DC == "DR" && grid.dataSource._data[no].Currency == "RS") {
            $(e.container).find('input[name="Credit"]').attr("readonly", true);
            $(e.container).find('input[name="Depit"]').focus();
        }
        else if (grid.dataSource._data[no].DC == "CR" && grid.dataSource._data[no].Currency != "RS") {
            $(e.container).find('input[name="Depit"]').attr("readonly", true);
            $(e.container).find('input[name="Credit"]').attr("readonly", true);
        }
        else if (grid.dataSource._data[no].DC == "DR" && grid.dataSource._data[no].Currency != "RS") {
    ...
    if (TransDate == "") {
     ...
     //   debugger;
        ...
        if (grdd != null || grddcd != null) {       
            ...
            if (grdd2.id == "Analysis" && grdd2 != null && grdd == null && grdd1 == null) {
                //boopathi
                //  debugger;
                var grid = $("#grid").data("kendoGrid");
                ...
                var Depit = grid._data[no].Depit;
                var Credit = grid._data[no].Credit;
                    ...
                    if (Depit != 0 || Credit != 0) {
                        ...
                        for (; item < total; item++) {
                            ...
                            if (rowIndex == RowIndexHiden) {
                                dataSource.add({ A_IND_VAl: A_IND_VAl, B_IND_VAl: B_IND_VAl, C_IND_VAl: C_IND_VAl, D_IND_VAl: D_IND_VAl, E_IND_VAl: E_IND_VAl, F_IND_VAl: F_IND_VAl, L_IND_VAl: L_IND_VAl, M_IND_VAl: M_IND_VAl, N_IND_VAl: N_IND_VAl, O_IND_VAl: O_IND_VAl, AnalysisAMt: AnalysisAMt });
                            }
                        }
                        var Checktotal = grid2.dataSource.data().length;
                        if (Checktotal == 0 || Checktotal == undefined) {
                            if (Depit != 0) {
                                $("#AnalysisTotal").val(Number(Depit).toFixed(2));
                                dataSource.add({ A_IND_VAl: "", B_IND_VAl: "", C_IND_VAl: "", D_IND_VAl: "", E_IND_VAl: "", F_IND_VAl: "", L_IND_VAl: "", M_IND_VAl: "", N_IND_VAl: "", O_IND_VAl: "", AnalysisAMt: Depit });
                            }
                            else {
                                $("#AnalysisTotal").val(Number(Credit).toFixed(2));
                                dataSource.add({ A_IND_VAl: "", B_IND_VAl: "", C_IND_VAl: "", D_IND_VAl: "", E_IND_VAl: "", F_IND_VAl: "", L_IND_VAl: "", M_IND_VAl: "", N_IND_VAl: "", O_IND_VAl: "", AnalysisAMt: Credit });
                            }
                        }
                        $("#AnalysisBalance").val(Number(0).toFixed(2));
                        //  grid2.addRow();                          
                        ...
                        $("#AlertMessageHdn").val("Depit or Credit Can not be Zero Value.");
                        $("#alertType").val('fail');
                         AlertMesaage();                       
                    }
                }
                else {
                    $("#AlertMessageHdn").val("Account Name Can not be Null Value.");
                    $("#alertType").val('fail');
                     AlertMesaage();                    
                }
            ...
            if (keyCode == 66 || keyCode == 98) {
                ...
                if (DepitVal != 0 || CreditVal != 0) {
                    if (grid.dataSource._data[no].BillDeatil_IND == 1) {
                        ...
                        var grid2 = $("#Popupgrid").data('kendoGrid');
                        var grid3 = $("#HiddenGrid").data('kendoGrid');
                        var ACID = grid.dataSource._data[no].AC_ID;
                        var dataSource = grid2.dataSource;
                        var total = grid3.dataSource.data().length;
                        if (total == undefined) {
                            total = 0;
...
$("#grid table").on("change", "td", function (e) {
   // debugger;
    ...
    $("#up").click(function () {
        ...
        $("#FinalDiffAMT").text("Debit/credit Difference: " + Number(TotalValue).toFixed(2));
...
$("#Property").change(function () {
   // debugger;
    var prpnm = $("#Property").val();
    $.ajax({
        type: "POST",
        url:"/GLTransaction/TransactionProNM",
        data: "id=" + prpnm,
        success: function (data) {                  
...
//Billl Detail..............
    ...
    function CancelRowPopup(e) {
        //    debugger;
        var AmountValuee = $("#AmountPop").val();
        $("#differenceAmount").text("Debit/credit Difference: " + Number(AmountValuee).toFixed(2));
        $("#PopupDepitTotal").val(Number(0).toFixed(2));
        $("#PopupCreditTotal").val(Number(0).toFixed(2));
        var grid1 = $("#Popupgrid").data("kendoGrid");
        ...
        if (total != 0) {
            ...
            if (AmountName == "DR") {
                ...
                var diffval = $("#differenceAmount").text();
                var dd = diffval.replace("Debit/credit Difference: ", "");
                diffval = dd;
                var AmountValuee = $("#AmountPop").val();
            ...
            $("#differenceAmount").text("Debit/credit Difference: " + Number(Difference).toFixed(2));
            var PopdepitfinTotal = $("#Popupgrid").data().kendoGrid.dataSource.aggregates().Depit.sum;
            var PopCreditfinTotal = $("#Popupgrid").data().kendoGrid.dataSource.aggregates().Credit.sum;
            $("#PopDepitTotal").text(Number(PopdepitfinTotal).toFixed(2));
            $("#PopCreditTotal").text(Number(PopCreditfinTotal).toFixed(2));
        ...
        if (diffval == "0.00") {
            ...
            if (validatae == true) {
                ...
                for (; item < total; item++) {
                    ...
                    dataSource2.add({ RowIndexHiden: RowIndex, A_IND_VAl: A_IND_VAl, B_IND_VAl: B_IND_VAl, C_IND_VAl: C_IND_VAl, D_IND_VAl: D_IND_VAl, E_IND_VAl: E_IND_VAl, F_IND_VAl: F_IND_VAl, L_IND_VAl: L_IND_VAl, M_IND_VAl: M_IND_VAl, N_IND_VAl: N_IND_VAl, O_IND_VAl: O_IND_VAl, AnalysisAMt: AnalysisAMt });
                }
                grid2.refresh();
